...
namespace JHSchool.Behavior.Report
{
    public static class Program
    {
        ...
        public static void Main()
        {
            ...
            Class.Instance.SelectedListChanged += delegate
            {
                ...
                else
                {
                    ...
                    Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Enable = Permissions.班級缺曠通知單權限 && Class.Instance.SelectedKeys.Count > 0;
                    Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Enable = Permissions.班級懲戒通知單權限 && Class.Instance.SelectedKeys.Count > 0;
                    Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Enable = Permissions.獎勵懲戒通知單權限 && Class.Instance.SelectedKeys.Count > 0;
            ...
            Student.Instance.SelectedListChanged += delegate
            {
                ...
                {
                    //20130227 - new
                    Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Enable = Permissions.學生懲戒通知單權限 && Student.Instance.SelectedKeys.Count > 0;
                    Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Enable = Permissions.學生缺曠通知單權限 && Student.Instance.SelectedKeys.Count > 0;
                    Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Enable = Permissions.學生獎懲通知單權限 && Student.Instance.SelectedKeys.Count > 0;
            ...
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Click += delegate
            {
                new 缺曠通知單.Report("class").Print();
            };

            //Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["懲戒通知單"].Enable = Permissions.班級懲戒通知單權限;
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Click += delegate
            {
                new 懲戒通知單.Report("class").Print();
            };

            //Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["獎勵懲戒通知單"].Enable = Permissions.獎勵懲戒通知單權限;
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Click += delegate
            {
            ...
            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Click += delegate
            {
                new 缺曠通知單.Report("student").Print();
            };

            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Click += delegate
            {
            ...
            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Click += delegate
            {
        ...
        private static void ClassFalse()
        {
            ...
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Enable = false;
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Enable = false;
            Class.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Enable = false;
        ...
        {
            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["懲戒通知單"].Enable = false;
            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["缺曠通知單"].Enable = false;
            Student.Instance.RibbonBarItems["資料統計"]["報表"]["學務相關報表"]["即將下架報表"]["獎懲通知單"].Enable = false;
