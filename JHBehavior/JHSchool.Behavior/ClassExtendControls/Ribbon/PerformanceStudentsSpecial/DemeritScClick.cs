...
namespace JHSchool.Behavior.ClassExtendControls.Ribbon
{
    class DemeritScClick
    {
        ...
        public void Print(CheckBoxX cbxSchoolYear1, IntegerInput intSchoolYear1,
            ...
            TextBoxX tbDemeritA, TextBoxX tbDemeritB, TextBoxX tbDemeritC, CheckBoxX cbxIsMeritAndDemerit)
        {
        ...
        void BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            //過Demerit
            int Demeritab = int.Parse(DSXConfig.GetText("Demerit/AB"));
            int Demeritbc = int.Parse(DSXConfig.GetText("Demerit/BC"));

            //過Demerit
            int Meritab = int.Parse(DSXConfig.GetText("Merit/AB"));
            int Meritbc = int.Parse(DSXConfig.GetText("Merit/BC"));
            ...
            foreach (JHStudentRecord student in StudentList)
            {
                ...
                foreach (AutoSummaryRecord each in AutoList[student.ID])
                {
                    foreach (JHDemeritRecord demerit in each.Demerits)
                    {
                        ...
                        //JHStudentRecord student = JHStudent.SelectByID(demerit.RefStudentID); //取得學生
            ...
            if (_cbxIsMeritAndDemerit)
            {
                ...
                titleCell_M.Style.Borders.SetColor(Color.Black);

                titleCell_M.PutValue(School.ChineseName + "　獎勵明細");
                titleCell_M.Style.HorizontalAlignment = TextAlignmentType.Center;
